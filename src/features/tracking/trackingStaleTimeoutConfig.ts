...
const stripInlineComment = (value: string): string => {
  ...
  // e.g. `30 # minutes`. Treat everything after `#` or `;`
  // as comment text.
...
export const getTrackingStaleTimeoutMs = (): number => {
  const raw = import.meta.env.VITE_TRACKING_STALE_TIMEOUT_MINUTES;
  const minutes = parseMinutes(raw) ?? DEFAULT_STALE_TIMEOUT_MINUTES;
